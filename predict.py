...
def data_save(i):
    if i>=1 and i<10:
        file_name = 'segs/00{}_seg.npy'.format(i)
    if i>=10 and i<100:
        file_name = 'segs/0{}_seg.npy'.format(i)
    if i>=100:
        file_name = 'segs/{}_seg.npy'.format(i)
    return file_name

def data_select(i):
    if i>=1 and i<10:
        file_name = '/content/drive/MyDrive/test_pub/test_pub/00{}_imgs.npy'.format(i)
#        file_name2 = 'train/00{}_seg.npy'.format(i)
    if i>=10 and i<100:
        file_name = '/content/drive/MyDrive/test_pub/test_pub/0{}_imgs.npy'.format(i)
#        file_name2 = 'train/0{}_seg.npy'.format(i)
    if i>=100:
        file_name = '/content/drive/MyDrive/test_pub/test_pub/{}_imgs.npy'.format(i)
#        file_name2 = 'train/{}_seg.npy'.format(i)
    a = np.load(file_name)
...
def interpolate(img, dimension=None):
    img = torch.Tensor(img) # to tensor
...
if __name__ == "__main__":
  ...
  # print(device)
  #model = UNet(in_dim=4, out_dim=1, num_filters=4)           ## num filters 4,8,16,32


  model = UNet(in_dim=4, out_dim=4, num_filters=8) ## filter change
  lr = 0.0001
  optimizer = optim.Adam(model.parameters(), lr) #SGD? ADAM?
  checkpoint = torch.load('model0_net52.npy')
  #checkpoint = torch.load('/content/drive/MyDrive/model0_net66.npy',map_location=torch.device('cpu'))
  ...
  for i in range(1, num_data+1):
     ...
     a, H, W, D = x.shape
     # print(x.shape)
     x = np.expand_dims(x,axis=0)   #[1,4,H,W,D]
     x = interpolate(x) #[1,4,128,160,128]

     x = x.to(device)

     y = model(x)     #[1,4,128,160,128]
     y = interpolate2(y, (H,W,D))   #[1,4,H,W,D]
     y = torch.squeeze(y, axis=0)  #[4,H,W,D]
     y = F.softmax(y, dim=0)       #[4,H,W,D]
     y = y.cpu().detach().numpy()
     y = y.argmax(0)  #[H,W,D]

     print(i)
     np.save(data_save(i), y)    ##how to save array outside?
